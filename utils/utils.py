...
def get_lab(x):
    xout = torch.zeros(x.shape)
    x = x.permute(0, 2, 3, 1)
    for i in range(0, x.shape[0]):
        one = torch.squeeze(x[i, ...], dim=0)
        one = one.cpu().numpy()
        img_lab = cv2.cvtColor(one, cv2.COLOR_BGR2LAB)
        img_lab = torch.from_numpy(img_lab).permute(2, 0, 1).unsqueeze(dim=0)
        xout[i,...]=img_lab
    return xout
