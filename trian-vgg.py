...
def main(args):
    ...
    model = vgg(num_classes=args.num_classes).to(device)

    if args.weights != "":
        assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
        weights_dict = torch.load(args.weights, map_location="cpu")
        del_keys = ['classifier.6.bias', 'classifier.6.weight']
        for k in del_keys:
            del weights_dict[k]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='vgg16-aug-all')
    ...
    parser.add_argument('--weights', type=str, default='/media/xjw/doc/00-ubuntu-files/download/vgg16-397923af.pth',
